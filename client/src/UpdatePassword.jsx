    ...
    <div className="fixed inset-0 bg-gray-900 bg-opacity-50 flex items-center justify-center p-4 z-50">
      ...
      <div className="bg-[#E0EBDD] p-0 rounded-none shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] w-full max-w-md border-2 border-black relative">
        ...
        <form onSubmit={handleUpdatePassword} className="space-y-4 p-8 pt-0">
          ...
          <div className="group relative">
            ...
            <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-3 w-64 p-3 bg-[#FFFFE1] border-2 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] text-xs hidden group-hover:block z-20">
              <p className="font-bold text-black mb-2 border-b border-black/20 pb-1">
                Security criteria:
              </p>
              <ul className="space-y-1">
                <li
                  className={`flex items-center gap-2 ${password.length >= 8 ? 'text-green-600 font-bold' : 'text-black/60'}`}
                >
                  {password.length >= 8 ? (
                    <Check size={12} />
                  ) : (
                ...
                <li
                  className={`flex items-center gap-2 ${/[A-Z]/.test(password) ? 'text-green-600 font-bold' : 'text-black/60'}`}
                >
                  {/[A-Z]/.test(password) ? (
                    <Check size={12} />
                  ) : (
                ...
                <li
                  className={`flex items-center gap-2 ${/[0-9]/.test(password) ? 'text-green-600 font-bold' : 'text-black/60'}`}
                >
                  {/[0-9]/.test(password) ? (
                    <Check size={12} />
                  ) : (
                ...
                <li
                  className={`flex items-center gap-2 ${/[^A-Za-z0-9]/.test(password) ? 'text-green-600 font-bold' : 'text-black/60'}`}
                >
          ...
          <div>
            ...
            <div className="relative">
              ...
              {confirmPassword && password === confirmPassword && (
                <Check
                  className="absolute right-3 top-3 text-emerald-500 animate-in fade-in zoom-in"
                  size={18}
                />
              )}
            </div>
        ...
        {title === 'Change your password' && (
          <button
            onClick={onDone}
            className="absolute top-2 right-2 text-black hover:bg-red-500 hover:text-white border-2 border-black bg-white w-8 h-8 font-mono rounded-sm shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] active:translate-x-[1px] active:translate-y-[1px] active:shadow-none transition-none"
          >
            &times;
          </button>
        )}

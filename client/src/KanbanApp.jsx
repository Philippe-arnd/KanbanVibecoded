          ...
          <div className="flex flex-col gap-4 w-full md:w-auto">
            <div className="flex justify-between items-center w-full md:w-auto gap-4">
              <div className="bg-black/10 p-1 rounded-sm inline-flex items-center self-start border-2 border-black">
                <button
                  onClick={() => setMode('pro')}
                  className={`px-4 py-1.5 rounded-sm text-sm font-bold flex items-center gap-2 transition-all ${isPro ? 'bg-white text-black shadow-[2px_2px_0px_0px_#00000080]' : 'text-black/70'}`}
                >
            ...
            <div>
              ...
              <div className="flex items-center gap-2 text-black/80 text-sm mt-1">
                {loading ? (
                  ...
                  <span>{visibleTasks.filter((t) => !t.completed).length} tasks to do</span>
                )}
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-col md:flex-row gap-2 md:items-center">
            <form onSubmit={handleAddTask} className="flex gap-2 items-center w-full md:w-auto">
              <input
                ...
                className="bg-white p-3 rounded-none w-full md:w-80 outline-none border-2 border-black focus:shadow-[2px_2px_0px_0px_#000] transition-none"
                placeholder={`New ${isPro ? 'Pro' : 'Personal'} task...`}
              />
              <button
                type="submit"
                className="bg-[#88D8B0] text-black p-3 rounded-none font-bold border-2 border-black shadow-[2px_2px_0px_0px_#000] active:translate-x-[1px] active:translate-y-[1px] active:shadow-none transition-none"
                disabled={loading}
              >
                <Plus />
              </button>

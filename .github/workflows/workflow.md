...
Every PR targeting main (non-draft)
  ├── pr-validation.yml         (lint/build + vitest + RLS tests + coverage report)
...
When any of the above complete
  └── auto-merge.yml            (squash merge if all 6 required checks pass)
```
...
|------|---------|
| 🔍 Lint | `npm run lint` |
| 🏗️ Build | `npm run build` (VITE_ENCRYPTION_KEY=ci-dummy-key) |

#### Job 2 — 🧪 Vitest Tests (timeout: 10m)
...
#### Job 4 — 📊 Test Report (`needs: [quick-checks, vitest-tests, rls-tests]`, `if: always()`)
...
#### Job 2 — 🛡️ Security Scan (timeout: 3m, BLOCKING on critical)
...
#### Job 4 — 📋 Security Report (`needs: [secret-detection, security-scan, bundle-size]`, `if: always()`)

- Consolidates all results into a PR comment (sentinel: `<!-- ci-security-report -->`)
- Bundle sizes displayed as `X KB / Y KB (Z%)` — e.g. `✅ 511 KB / 600 KB (85%)`
